...
<YYINITIAL> {
   [0-9]+ { return new Symbol(sym.INT_LITERAL, Integer.valueOf(yytext())); }
    [0-9]+"."[0-9]*([eE][+-]?[0-9]+)?[fF]?   {
        return new Symbol(sym.FLOATING_POINT_LITERAL, Float.valueOf(yytext()));
    }

    [0-9]+[eE][+-]?[0-9]+[fF]? {
        return new Symbol(sym.FLOATING_POINT_LITERAL, Float.valueOf(yytext()));
    }
    0[0-7]+    { return new Symbol(sym.INT_LITERAL, Integer.valueOf(yytext(), 8)); }
    0[xX][0-9a-fA-F]+   { return new Symbol(sym.INT_LITERAL, Integer.valueOf(yytext().substring(2), 16)); }
